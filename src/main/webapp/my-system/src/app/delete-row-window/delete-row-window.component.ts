...
@Component({
  selector: 'app-delete-row-window',
  templateUrl: './delete-row-window.component.html',
  styleUrls: ['./delete-row-window.component.css']
})
export class DeleteRowWindowComponent implements OnInit {
  tableForm: FormGroup;
  submitted = false;
  selected = '';
  user: User = null;

  constructor(private formBuilder: FormBuilder, private router: Router,  private tableService: TableService,  private service: LoginAndRegistrate, private taskService: TaskService)  {
  ...
  getData() {
    ...
    if (this.tableForm.invalid) {
      ...
      this.tableService.deleteUser(user).subscribe((answer) => {
      ...
      this.tableService.deleteStore(store).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
    } else if (this.selected === 'sport') {
      const sport = new Sport();
      sport.name = this.tableForm.value['sport_name'];
      this.tableService.deleteSport(sport).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteShop(shop).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
    } else if (this.selected === 'season') {
      const season = new Season();
      season.season = this.tableForm.value['season_season'];
      this.tableService.deleteSeason(season).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteSale(sale).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteOnlineSale(onlineSale).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteGoodSize(goodSize).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteGoodInStore(goodInStore).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteGoodInShop(goodInShop).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteGoodInOrder(goodInOrder).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteGood(good).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteEmployee(employee).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteDelivery(delivery).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
      ...
      this.tableService.deleteCheque(cheque).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
    } else if (this.selected === 'brand') {
      const brand = new Brand();
      brand.brand = this.tableForm.value['brand_brand'];
      this.tableService.deleteBrand(brand).subscribe((answer) => {
        this.showInfo(answer.toString());
      });
    }
  ...
  showInfo(answer: string) {
    if (answer === 'OK') {
      ...
      alert('Ошибка! Данных не было найдено для удаления!');
    }
  }
}
